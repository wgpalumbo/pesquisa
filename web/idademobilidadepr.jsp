...
<head>
	...
	function EnviaExibir(){
		document.frmcad.action = "idademobilidadepr.jsp";
...
<% 
	...
	ArrayList<String> lstMotivos = relato.Idades();
	ArrayList<String> lstModos = relato.ModosPR();
	HashMap<String, Integer> mapMotModo = relato.MontaIdadeModosPR(ano);	
	HashMap<String, Integer> mapTotFaixa = relato.MontaTotalFaixa(ano);	
	relato=null;
	int totalzaolinha=0;
	double mobilzao=0.0;
	int totalfaixa=0;
	...
	if(!mapMotModo.isEmpty()&&!lstMotivos.isEmpty()){
		...
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Mobilidade x Faixa Et&aacute;ria</font></td></tr><tr><td>");
		//------		
		out.println("<table align='center' border='1' cellpadding='2' cellspacing='0' bordercolor='#DDDDDD' bgcolor='#FFFFFF'>");
		out.println("<tr align='center' bgcolor='#6699CC'>");
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Faixa Et&aacute;ria</font></td>");
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Soma/Faixa</font></td>");
		colunasXLS.add("Faixa Etaria");
		...
		for(int j=0;j<lstModos.size();j++){
			...
			try{
				...
				mobilinha = Math.round(mobilinha * 100);
				mobilinha = mobilinha / 100;
			} catch(Exception e){ }
			out.println("<td nowrap  colspan=2 align='center' bgcolor='#6699CC'><font face="+fonteTTF+" size='2' color='white'><b>"+(totalcoluna[j]>0?String.valueOf(totalcoluna[j]):" ")+"</b> / <font color='yellow'>"+(mobilinha>0?nrodecimal.format(mobilinha):" ")+"</font></font></td>");
			planilhaExcel += (totalcoluna[j]>0?String.valueOf(totalcoluna[j]):" ") +";";
		}
		try{
			mobilzao = (double)totalzaolinha/totalfaixa;
			mobilzao = Math.round(mobilzao * 100);
			mobilzao = mobilzao / 100;
		} catch(Exception e){ }
...
<%
	//------------------
	String nomeArquivo="ModoPrincipalxIdade";
	if(ano.length()>0){
