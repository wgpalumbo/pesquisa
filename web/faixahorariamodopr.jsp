...
function EnviaExibir(){
	document.frmcad.action = "faixahorariamodopr.jsp";
...
<% 
	...
	ArrayList<String> lstMotivos = relato.FxHoraria();
	ArrayList<String> lstModos = relato.ModosPR();
	HashMap<String, Integer> mapMotModo = relato.MontaFxHorariaModosPRPorZonas(zona, ano, semfator);	
	relato=null;
	int totalzaolinha=0;
	...
	if(!mapMotModo.isEmpty()&&!lstMotivos.isEmpty()){
		...
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Fx.Horaria x Modos</font></td>");		
		out.println("</tr><tr><td>");
		//------		
		out.println("<table align='center' border='1' cellpadding='2' cellspacing='0' bordercolor='#DDDDDD' bgcolor='#FFFFFF'>");
		out.println("<tr align='center' bgcolor='#6699CC'>");
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Fx.Horaria</font></td>");
		colunasXLS.add("Fx.Horaria");
		for(int i=0;i<lstModos.size();i++){
			String iModo = lstModos.get(i);
			out.println("<td nowrap align='center'><font face="+fonteTTF+" size='2' color='white'>"+iModo+"</font></td>");
...
<%
	//------------------
	String nomeArquivo="FxHorariaxModos_Principal_"+ano;
	if(zona.length()>0){
