...
<head>
	...
	function EnviaExibir(){
		document.frmcad.action = "modosfxrendmobilidadepr.jsp";
...
<% 
	...
	nrodecimal.setMaximumFractionDigits(2);
	nrodecimal.setMinimumFractionDigits(2);
	//-----
	Relatorios relato = new Relatorios(pageContext); 
	ArrayList<String> lstModos = relato.ModosPR();
	HashMap<String, Integer> mapMotModo = relato.MontaFxRendaModosPR(ano);	
	ArrayList<String> lstFxRenda = relato.lstFxRenda;
	HashMap<String, Integer> mapTotFaixa = relato.MontaTotalRenda();	
	relato=null;
	int totalfaixa=0;
	int totalzaolinha=0;
	double mobilzao=0.0;
	...
	if(!mapMotModo.isEmpty()&&!lstFxRenda.isEmpty()){
		...
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Mobilidade x Faixa Renda</font></td></tr><tr><td>");
		//------		
		...
		for(int i=0;i<lstFxRenda.size();i++){ 
			...
			//----
			int soma=0;
			if (mapTotFaixa.containsKey(iMotivo)) {
				soma = mapTotFaixa.get(iMotivo);
			}
			totalfaixa+=soma;
			...
			for(int j=0;j<lstModos.size();j++){
				...
				double mobil = 0;
				try{
					if(soma>0)mobil = (double)conta/soma;
					mobil = Math.round(mobil * 100);
					mobil = mobil / 100;
				} catch(Exception e){ }
				mobilinha+=mobil;
				//----
			...
			try{
				mobilinha = (double)totallinha/soma;
...
<%
	//------------------
	String nomeArquivo="ModoPrincipalxFaixadeRenda";
	if(ano.length()>0){
