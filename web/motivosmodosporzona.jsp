...
function EnviaExibir(){
	document.frmcad.action = "motivosmodosporzona.jsp";
...
<% 
	...
	HashMap<String, Integer> mapMotModo = relato.MontaMotivosModosPorZonas(zona, ano, semfator);	
	//for (Map.Entry<String, Integer> set : mapMotModo.entrySet()) {
		    //out.println(set.getKey() + " = " + set.getValue()+"<br>");
	//}
	relato=null;
	int totalzaolinha=0;
	...
	if(!mapMotModo.isEmpty()&&!lstMotivos.isEmpty()){
		...
		out.println("<td nowrap><font face="+fonteTTF+" size='2' color='white'>Motivos x Modos / Zona "+zona+" como "+(sentido.equals("O")?"Origem":"Destino")+"</font></td>");
...
<%
	//------------------
	String nomeArquivo="MotivosxModos_Simples_"+ano;
	if(zona.length()>0){
